...
def get_model():
    global _model
    if _model is None:
        name = os.getenv("EMBED_MODEL","BAAI/bge-base-en")
...
def get_reranker():
    """Get cross-encoder reranker model with fallback"""
    global _reranker
    if _reranker is None:
        try:
            # Try bge-reranker-large first, fallback to smaller model
            reranker_name = os.getenv("RERANK_MODEL", "cross-encoder/ms-marco-MiniLM-L-6-v2")
            if "bge-reranker" in reranker_name:
...
def score_pairs(query: str, passages: List[str]) -> List[float]:
    ...
    if reranker is None:
        # Fallback: return dummy scores (all zeros)
        logger.warning("Reranker not available, returning dummy scores")
